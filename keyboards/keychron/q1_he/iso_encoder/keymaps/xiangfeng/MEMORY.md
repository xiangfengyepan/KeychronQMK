...
|--------|------:|---:|------------|
| `.text` | 109,064 | **~106 KB** | program code (QMK core + all custom logic) |
| `.rodata` | 48,384 | **~47 KB** | read-only constants — **includes the IME dictionary ≈ 41 KB** |
...
| **firmware content** | **161,036** | **~157 KB** | sum of the above |
| reserved (app offset) | 32,312 | ~32 KB | gap before code — `.text` starts at `0x0800_8000` |
| **flashed image (`.bin`)** | **193,348** | **~189 KB** | what actually gets written to flash |

So of the 256 KB: **~189 KB is the flashed image**, **~12 KB** is the emulated-EEPROM slice, leaving
**~55 KB free**. The dictionary (`.rodata`, ~41 KB) is the biggest single thing *you* added.
...
**How to read it:**
- **`.text` (~106 KB)** = compiled program. **`.rodata` (~47 KB)** = read-only constants — the
...
- Add features/data → spends **Flash**. Watch the ~55 KB free (≈ another ~200 dictionary chars at
  ~0.26 KB each).
