...
Source: `letters.c` in this folder (registered as **USER** effects in `rgb_matrix_user.inc`).
...
| **Spider-Man** | 27 | Red mask with two white angular eyes (occasional blink); each keypress fires a quick white web-burst. **Power-on default** (`RGB_MATRIX_DEFAULT_MODE`). Source: `spider_mask.c` in this folder. |
